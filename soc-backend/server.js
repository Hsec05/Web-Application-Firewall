...
async function runScheduledReports() {
  try {
    ...
    console.log(`📄  Scheduled report generated: ${pdf.length} bytes. Preparing to email...`);

    // 1. Configure the email server connection
    ...
    // 2. Draft the email and attach the PDF
    const mailOptions = {
      from: `"SOC Dashboard" <${process.env.SMTP_USER}>`,
      to: email, // This is pulled from your DB settings
      subject: `Automated WAF Report: ${type}`,
      text: `Hello,\n\nPlease find attached the scheduled ${type} security report generated by the SOC Dashboard.\n\nBest,\nSecurity Team`,
      attachments: [
        {
          filename: `WAF-Report-${type}-${new Date().toISOString().split('T')[0]}.pdf`,
          content: pdf, // This takes the PDF buffer directly from memory!
    ...
    if (email) {
      ...
      console.log(`⚠️  Report generated, but no recipient email was configured in settings.`);
    }
